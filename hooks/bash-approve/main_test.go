...
func TestEvaluate_Approved(t *testing.T) {
	tests := []struct {
		...
		{"nix-shell --command read-only", "nix-shell -p go --command 'go test ./...'", "nix-shell"},
		{"nix-build", "nix-build -A pkgs.hello", "nix-build"},
		{"nix eval", "nix eval '.#nixosConfigurations.falcon.config.system.name'", "nix"},
		{"nix log", "nix log '.#hello'", "nix"},
		{"nix path-info", "nix path-info -r '.#hello'", "nix"},
		{"nix flake show", "nix flake show", "nix flake read"},
		{"nix flake metadata", "nix flake metadata", "nix flake read"},
		{"nix flake info", "nix flake info", "nix flake read"},
		{"nix flake update", "nix flake update", "nix flake read"},
		{"nix-store query", "nix-store -q --references /nix/store/abc", "nix-store query"},
		...
		{"fd by extension", "fd -t f '\\.test\\.ts$' frontend/src/lib/", "fd"},
		{"fd plain pattern", "fd '\\.go$' .", "fd"},
		{"fdfind alias", "fdfind -t d", "fd"},
		{"fd multi paths", "fd -t f '\\.test\\.ts$' frontend/src/lib/components/content/ frontend/src/lib/stores/", "fd"},
		{"fd -x rg", "fd -t f '\\.go$' . -x rg pattern {}", "fd"},
		{"fd -X wc", "fd -t f '\\.go$' . -X wc -l", "fd"},
